...
public class main {
	...
	public static void main(String[] args) {
		...
		numberOfFeatures = 6;
		k = 3; // number of clusters, should be empirically set
		...
		singleData = readCSV("temperature_byhour", 1);
		...
		bulkData = readCSVBulk("temperature_byhour");
		...
		for (int a = 0; a < anomalyArray.size(); a++) {
			...
			//int cluster = findCluster(v, "electricity_byminute_attributenames",
				//	arffData);
			int cluster = findCluster(v, "temperature_byhour",
					arffData);
...
//		return false;
	...
	public static int getDayOfWeek(String value)
	{
		...
		try {
			c.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse(value));
